...
ofPoint pBot = ofPoint(100, -100);  // start
ofPoint pRi = ofPoint(-100, -100); // go up and right
//ofPoint pLe = ofPoint(50, -50);  // go up and left
//ofPoint pTop = ofPoint(-50, 0); // go down and left
//ofPoint pEnd = ofPoint(50, 0);  // go down and right   it's a circle!
ofPoint gPoints[] = {pBot, pRi};

ofVideoPlayer myPlayer[3];
...
void ofApp::setup(){
    ...
    for(int i=0; i<3; i++){
        ...
        myPlayer[i].load(to_string(i)+".mp4");
        myPlayer[i].play();
    }
    frameByframe = false;
...
void ofApp::draw(){
    ...
    myPlayer[num].draw(-512,-360,ofGetWindowWidth(),ofGetWindowHeight());
...
//            ofLine(dip.x, dip.y, dip.z, tip.x, tip.y, tip.z);
            ...
            if(dist>100){
                //ofSetColor(255);
                //ofDrawSphere(0, 0, 80, 20);
                frameByframe=!frameByframe;
                myPlayer[num].setPaused(frameByframe);
            }
//            cout<<dist<<endl;
            }
